...
| **v0.5.0** | **Spring Boot 4 release line.** SB4 / Spring Framework 7 / Jackson 3 baseline. PageHelper 4.0.0 + MyBatis 4.0.1. `spring-boot-starter-aop` → `spring-boot-starter-aspectj`. Starter auto-registers `PageableHandlerMethodArgumentResolver` + `SortHandlerMethodArgumentResolver`. |
...
Shape will be driven by post-4.0.0 user feedback. Candidates likely to land here come from the [Considering](#considering--not-committed) section — open an issue to vote on what you want next.
