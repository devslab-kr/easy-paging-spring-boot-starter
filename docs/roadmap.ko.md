...
| 릴리스 | 주요 변경 |
|---|---|
| **v4.0.0** | **Spring-major-정렬 버전 정책** — `0.5.0`의 재번호링. 라이브러리 메이저가 이제 Spring Boot 메이저와 일치 (4.x = SB4, 3.x = SB3). 코드 동일, 좌표만 새로. [VERSIONING 정책](https://github.com/devslab-kr/.github/blob/main/.github/VERSIONING.md#한국어) 참조. |
| **v3.0.0** | Spring Boot 3 maintenance 라인을 `0.4.0`에서 재번호링. 코드 동일, 좌표만 새로. SB 3.3–3.5 maintenance가 여기서 계속, 보안 패치 제공. |
| **v0.5.0** | **Spring Boot 4 릴리즈 라인.** SB4 / Spring Framework 7 / Jackson 3 베이스라인. PageHelper 4.0.0 + MyBatis 4.0.1 (SB4 호환). `spring-boot-starter-aop` → `spring-boot-starter-aspectj`. 스타터가 이제 `PageableHandlerMethodArgumentResolver` + `SortHandlerMethodArgumentResolver`를 자동 등록 (SB4가 auto-config 제거). |
...
v4.0.0 이후 사용자 피드백에 따라 형태가 결정됩니다. 현재 [검토 중](#검토-중--미확정) 섹션의 항목들이 후보 — 원하는 항목에 이슈로 투표해주세요.

`3.x` maintenance 브랜치는 Spring Boot 3.5 자체가 upstream에서 지원받는 동안 보안 패치와 의존성 bump를 계속 받습니다.
